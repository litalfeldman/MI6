...
public abstract class Subscriber extends RunnableSubPub {
    ...
    private MessageBroker M_B;
    private HashMap<Class<? extends Message>, Callback> map_type_callBack;
    ...
    public Subscriber(String name) {
        super(name);
        M_B = MessageBrokerImpl.getInstance();
        map_type_callBack = new HashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        M_B.subscribeEvent(type, this);
        map_type_callBack.put(type, callback);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        M_B.subscribeBroadcast(type, this);
        map_type_callBack.put(type, callback);
    }
    ...
    public final void run() {
        M_B.register(this);
        initialize();
        subscribeBroadcast(DieUallMotherfuckersBroadcast.class,(b)->{terminate();});
        MI6Runner.countDownLatch.countDown();
        try {
            while (!terminated) {
                Message message = M_B.awaitMessage(this);
                Callback callback = map_type_callBack.get(message.getClass());
